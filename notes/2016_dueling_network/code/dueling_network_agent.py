...
class DuelingNetwork(nn.Module):
    def __init__(self, n_frame, n_actions):
        ...
        # 利得関数の流れ
        self.advantage_fc = nn.Linear(256, n_actions)
        ...
        torch.nn.init.kaiming_normal_(self.advantage_fc.weight)
...
class DuelingNetworkAgent():
    ...
    def initialize(self):
        self.model = DuelingNetwork(self.n_frame, len(self.actions)).to(self.device)
        self._teacher_model = DuelingNetwork(self.n_frame, len(self.actions)).to(self.device)
    ...
    def load(cls, actions, model_path, device, n_frame=4, epsilon=0.0001):
        ...
        agent.model = DuelingNetwork(n_frame, len(actions)).to(device)
