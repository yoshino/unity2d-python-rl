...
from .dueling_network_agent import DuelingNetworkAgent
from .replay_buffer import ReplayBuffer


class DuelingNetworkTrainer():
    ...
    def train(self, env):
        agent = DuelingNetworkAgent(epsilon=self.epsilon_scheduler(self.total_steps), actions=list(range(env.action_space.n)), device=self.device)  
        observe_interval = 0
    ...
    def train_loop(self, env, agent):
        for i in range(self.episode_count):
            ...
            while not done:
                ...
                if len(self.replay_buffer.buffer) > 50000:
                    # モデルの初期化
