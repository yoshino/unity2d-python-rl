...
class NoisyNetwork(nn.Module):
    def __init__(self, n_frame, n_actions):
        ...
        self.fc1 = FactorizedNoisy(64 * 7 * 7, 256)
        self.fc2 = FactorizedNoisy(256, n_actions)
...
class NoisyNetworkAgent():
    ...
    def initialize(self):
        self.model = NoisyNetwork(self.n_frame, len(self.actions)).to(self.device)
        self._teacher_model = NoisyNetwork(self.n_frame, len(self.actions)).to(self.device)
    ...
    def load(cls, actions, model_path, device, n_frame=4, epsilon=0.0001):
        ...
        agent.model = NoisyNetwork(n_frame, len(actions)).to(device)
